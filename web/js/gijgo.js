...
gj.core = {
    ...
    setCaretAtEnd: function (elem) {
        ...
        if (elem) {
            ...
            elemLen = elem.innerHTML.length;  //DENIS: так как мы используем div, а не textarea
...
gj.grid.methods = {
    ...
    loadData: function ($grid) {
        ...
        /*for (i = 0; i < rowCount; i++) {  //DENIS: Чтобы не было ошибок в связи с использованием функции prepend в методе renderRow
            if (i < recLen) {
                $row = $rows.eq(i);
        ...
        $tbody.find('tr[data-role="row"]').remove();    //DENIS: Очищаем таблицу
        for (i = 0; i < recLen; i++) {
            gj.grid.methods.renderRow($grid, null, records[i], i);  //DENIS: Вставляем обновленные записи в таблицу
        }
    ...
    renderRow: function ($grid, $row, record, position) {
        ...
        if (!$row || $row.length === 0) {
            ...
            //$grid.children('tbody').append($row); //DENIS: Чтобы добавлять новую строку в начало, а не в конец, используем prepend
            $grid.children('tbody').prepend($row);
        } else {
            mode = 'update';
    ...
    getRowById: function ($grid, id) {
        ...
        if (primaryKey) {
            for (i = 0; i < records.length; i++) {
                if (records[i][primaryKey] == id) {
                    position = i + 1;
                    break;data.dataSource.push(record);
                }
            }
        } else {
            position = id;
        ...
        for (field in data.params) {
            if (data.params[field] && !data.paramNames[field]) {
                ...
                records = $.grep(records, function (record) {
                    ...
                    //return column && typeof (column.filter) === 'function' ? column.filter(value, searchStr) : (value.toString().toUpperCase().indexOf(searchStr.toString().toUpperCase()) > -1);    //DENIS: чтобы избежать ошибки, когда фильтруется нетекстовая переменная
                });
            }
        }
    ...
    addRow: function ($grid, record) {
        ...
        $grid.edit(record.Id);    //DENIS: автоматически редактируем вновь созданную запись
        return $grid;
    },
    ...
    count: function ($grid, includeAllRecords) {
        ...
        return includeAllRecords ? $grid.data().totalRecords : typeof $grid.getAll() === 'undefined' ? 0 : $grid.getAll().length; //DENIS: Чтобы не было ошибки, когда grid не инициализировался.
    },
    ...
    renderers: {
        editManager: function (value, record, $cell, $displayEl, id, $grid) {
            ...
            $delete.on('click', function (e) {
                if (confirm('Удалить запись?')) {   //DENIS: Исправил баг https://github.com/atatanasov/gijgo/issues/366
...
gj.grid.plugins.inlineEditing.private = {
    localization: function (data) {
        if (data.uiLibrary === 'bootstrap') {
            ...
            //data.inlineEditing.editButton = '<button role="edit" class="gj-button-md"><i class="gj-icon pencil" /> ' + gj.grid.messages[data.locale].Edit.toUpperCase() + '</ button>'; //DENIS: Оставим просто иконки
            data.inlineEditing.editButton = '<button role="edit" class="gj-button-md"><i class="gj-icon pencil" /></button>';
            ...
            data.inlineEditing.deleteButton = '<button role="delete" class="gj-button-md"><i class="gj-icon delete" /></button>';
            ...
            data.inlineEditing.updateButton = '<button role="update" class="gj-button-md"><i class="gj-icon check-circle" /></button>';
            ...
            data.inlineEditing.cancelButton = '<button role="cancel" class="gj-button-md"><i class="gj-icon cancel" /></button>';
        }
    },
    ...
    editMode: function ($grid, $cell, column, record) {
        ...
        if ($cell.attr('data-mode') !== 'edit') {
            if (column.editor) {
                ...
                $editorField = $editorContainer.find('input, select, textarea, div').first();   //DENIS: добавим обработку div
                if ($editorField.length) {
                    ...
                    if (typeof (column.editor) === 'function') {
                        ...
                        if ('checkbox' === column.type && gj.checkbox) {
                            ...
                            //$editorField = $('<input type="text" value="' + value + '" class="gj-width-full"/>'); //DENIS: заменим input на textarea
                            //$editorField = $('<textarea id="editor">'+value+'</textarea>');
                            $editorField = $('<div contenteditable>'+value+'</div>');

                            if (data.uiLibrary === 'materialdesign') {
                                //$editorField.addClass('gj-textbox-md').css('font-size', $grid.css('font-size'));  //DENIS: уберем нижнюю черту в классе gj-textbox-md
                                //$editorField.addClass('gj-textbox-md').css({'font-size': $grid.css('font-size'), 'resize':'none', 'overflow':'hidden', 'line-height':'normal', 'padding':'12 0 12 6'});   //DENIS: для textarea
                                $editorField.addClass('gj-textbox-md').css({'font-size': $grid.css('font-size'), 'line-height':'normal', 'padding':'12 0 12 6', 'white-space':'normal'}); //DENIS: для div
                            }
                            var hashtag = false;

                            $editorField.on('keyup input focus', function(e) {
                                //$(this).css('height', '0px').css('height', this.scrollHeight);  //DENIS: теперь высота textarea зависит от содержимого
                                if (e.ctrlKey && e.keyCode == 13) { $grid.update(record.Id) }   //DENIS: обновляем по ctrl+Enter
                                else if (e.keyCode == 27) { $grid.cancel(record.Id) }   //DENIS: отменяем по Esc
                                else if (e.shiftKey && e.keyCode == 51) { //DENIS: нажимаем shift+3
                                    hashtag = true;
                                }
                                else if (e.keyCode == 32 && hashtag && $(this).html().indexOf("#") != -1) { //DENIS: нажимаем пробел
                                    //https://stackoverflow.com/questions/3926451/how-to-match-but-not-capture-part-of-a-regex
                                    //$editorField.html($editorField.html().replace(/([^?!>])#([^ |&]+)/gm, "<a href=javascript:;>#$2</a>"));
                                    //?<! - заглянуть за > и найти только (чтобы не накладывать ссылки на ссылки)
                                    //|& чтобы не захватывать неразрывные проблеы &nbsp;
                                    var regexp = new RegExp("(?<!>)#([^ |&]+)","gm");
                                    $editorField.html($editorField.html().replace(regexp, "<a href=javascript:; onclick='hashf(this)'>#$1</a>"));
                                    gj.core.setCaretAtEnd(this);
                                    hashtag = false;
                                }
                            });
                            $editorField.on('paste', function(e) { //DENIS: Чтобы вместо "форматированного текста" вставлялся обычный текст
                                e.preventDefault();
                ...
                if ($editorField.prop('tagName').toUpperCase() === "INPUT" && $editorField.prop('type').toUpperCase() === 'TEXT') {
                    //gj.core.setCaretAtEnd($editorField[0]);   //DENIS: Нам не нужен фокус на input'ах
                } else {
                    //$editorField.focus(); //DENIS: Нам не нужен фокус в начале textarea
                    gj.core.setCaretAtEnd($editorField[0]); //DENIS: Нам нужен фокус в конце textarea или div
                }
    ...
    displayMode: function ($grid, $cell, column, cancel) {
        ...
        if (column.mode !== 'editOnly') {
            if ($cell.attr('data-mode') === 'edit') {
                ...
                $ele = $editorContainer.find('input, select, textarea, div').first();   //DENIS: добавим обработку div
    ...
    updateOtherCells: function($grid, mode) {   //DENIS: Функция вызывается при редактировании данных
        var data = $grid.data();
        $('#btnAddFile').prop('disabled', false);   //DENIS: включаем кнопку
        $('#btnAddLink').prop('disabled', false);   //DENIS: включаем кнопку
...
gj.grid.plugins.inlineEditing.public = {
     ...
     * </script>
     */
    update: function (id) { //DENIS: Функция вызывается после сохранения изменений!
        var i, record = this.getById(id),
            $cells = gj.grid.methods.getRowById(this, id).children('td'),
            columns = this.data('columns');
        ...
        $('#btnAddFile').prop('disabled', true);   //DENIS: отключаем кнопку
        $('#btnAddLink').prop('disabled', true);   //DENIS: отключаем кнопку
     ...
     * </script>
     */
    cancel: function (id) { //DENIS: функция вызывается в самом начале загрузки страницы, при отмене изменений и при добавлении/удалении строки
        var i, record = this.getById(id),
            $cells = gj.grid.methods.getRowById(this, id).children('td'),
            columns = this.data('columns');
        for (i = 0; i < $cells.length; i++) {
    ...
    private: {
        init: function ($grid) {
            ...
            $grid.on('rowDataBound', function (e, $row, id, record) {
                ...
                //if ( previousValue !== record[data.grouping.groupBy] || $row.attr('data-position') == data.records.length) { //DENIS: Копать где-то здесь
